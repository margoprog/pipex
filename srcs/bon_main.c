...
/*   Updated: 2023/04/14 02:45:21 by maheraul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
						0			1			2			3			4
./pipex	"infile"	"cat -e" |	"grep a" |	"lsblk"		"echo aaa"	"wc
								-l" "outfile"
...
int	redirection(t_data *data, int index, char **av)
{
	...
	if (index == 0) // 0
	{
		infilefd = open(av[index + 1], O_RDONLY);
		if (infilefd == -1)
	...
	{
		outfilefd = open(av[index + 3], O_WRONLY | O_CREAT | O_TRUNC);
		if (outfilefd == -1)
...
void	*ft_pipex(t_data *data, char **av, char **env)
{
	...
	while (i < data->nbcmd)
	// i = 0 : cmd = cat ; i = 1 : cmd = grep ; i = 2 : cmd = wc -l
	{
...
// {
// 	if (i == 0)
// 		execve("/bin/echo", (char *[3]){"/bin/echo",
				"cette phrase contient multiple fois la lettre e", NULL}, env);
// 	if (i == 1)
